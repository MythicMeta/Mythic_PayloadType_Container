...
async def initialize_task(
    ...
    task = MythicCommandBase.MythicTask(
        ...
        args=command_class.argument_class(
            command_line=message_json["params"],
    ...
    async with message.process():
        ...
        if command == "create_payload_with_code":
            try:
                ...
                message_json = json.loads(message.body, strict=False)
                ...
                for cls in MythicCommandBase.CommandBase.__subclasses__():
                    if getattr(cls, "cmd") == message_json["command"]:
                        ...
                        if task is None:
                            return
                        if (
                            "opsec_class" in dir(Command)
                            and Command.opsec_class is not None
                            and callable(Command.opsec_class.opsec_pre)
                        ):
                            ...
                            if task.opsec_pre_blocked is None:
                                ...
                                task.opsec_pre_blocked is True
                                and not task.opsec_pre_bypassed
                            ):
                        ...
                        if (
                            "opsec_class" in dir(Command)
                            and Command.opsec_class is not None
                            and callable(Command.opsec_class.opsec_post)
                        ):
                            ...
                            if task.opsec_post_blocked is None:
                                ...
                                try:
                                    ...
                                    return
                                if (
                                    task.opsec_post_blocked
                                    and not task.opsec_post_bypassed
                                ):
                        ...
                        await send_status(
                            ...
                            if final_task.status != "preprocessing"
                            else "success",
                            reference_id=reference_id,
                            username=username,
                        )
            ...
            try:
                ...
                for cls in MythicCommandBase.CommandBase.__subclasses__():
                    if getattr(cls, "cmd") == message_json["command"]:
                        ...
                        if task is None:
                            return
                        agentResponse = MythicCommandBase.AgentResponse(
                            task=task, response=message_json["response"]
                        )
            ...
            print_flush(
                "[*] If this is unexpected, check the Mythic UI for logs as to why this container is tasked to exit",
                override=True,
            )
            sys.exit(0)
            ...
            try:
                ...
                for cls in MythicCommandBase.CommandBase.__subclasses__():
                    if getattr(cls, "cmd") == message_json["command"]:
                        ...
                        if task is None:
                            return
                        if hasattr(Command, message_json["function_name"]) and callable(
                            getattr(Command, message_json["function_name"])
                        ):
                            # the message_json["function_name"] function is defined in the Command.callback_class and is callable
                            try:
                                final_task = await getattr(
                                    Command, message_json["function_name"]
                                )(
                                    task,
                                    message_json["subtask"],
                                    message_json["subtask_group_name"],
                                )
...
async def sync_classes(reference_id: str = ""):
    try:
        ...
        print_flush(
            "[-] Failed to sync classes, exiting container!\n"
            + str(traceback.format_exc()),
            override=True,
        )
        await send_status(
...
async def connect_to_rabbitmq():
    ...
    if not os.path.exists(container_files_path):
        print_flush(
            f"[-] Specified mythic_container_files_path, {container_files_path},  does not exist",
            override=True,
        )
        print_flush(
            f"[-] If this isn't in a Docker container, then this should be the path to your PayloadType folder",
            override=True,
        )
...
async def mythic_service():
    ...
    try:
        print_flush(
            "[*] mythic_service - Waiting for messages in mythic_service with version {}.".format(
                container_version
            ),
            override=True,
        )
        ...
        print_flush(
            "[*] Now parsing agent_functions python files to sync with Mythic server"
        )
        await sync_classes()
...
async def rabbit_mythic_rpc_callback(
    ...
    async with message.process():
        try:
            ...
            response = {
                "status": "error",
                "error": "command's dynamic_query_function not found or not callable",
            }
            for cls in MythicCommandBase.CommandBase.__subclasses__():
                if getattr(cls, "cmd") == message_json["command"]:
                    Command = cls(Path(container_files_path))
                    ...
                    for command_param in CommandArgs.args:
                        if command_param.name == message_json["action"] and callable(
                            command_param.dynamic_query_function
                        ):
                            response = await command_param.dynamic_query_function(
                                message_json["callback"]
                            )
                    break
        except Exception as e:
            print_flush(
                "[-] Exception trying to respond to dynamic_query_function:\n"
                + str(traceback.format_exc()),
                override=True,
            )
...
async def connect_and_consume_mythic_rpc():
    ...
    try:
        ...
        print_flush(
            "[-] Exception in connect_and_consume_mythic_rpc .consume: {}".format(
...
def start_service_and_heartbeat(**kwargs):
    ...
    if "debug" in kwargs:
        print_flush(
            "[*] the `debug` argument is deprecated, instead set the `MYTHIC_ENVIRONMENT` variable directly for the container or for Mythic/.env",
            override=True,
        )
    ...
    if "debug" in kwargs and kwargs["debug"]:
        ...
        print_flush(
            "[*] Setting MYTHIC_ENVIRONMENT=Testing based on deprecated debug=True argument",
            override=True,
        )
